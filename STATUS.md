...
## Resumen 1-line
**Batch jugabilidad 2026-06-08 ✅ DONE** (rama `f5d-map-pixi-osm`, HEAD `9b44955`) — Tanda de features pedidas por Dani sobre el juego vivo: **(1) Velocidades** 1×/5×/10×/25× + test 500×/5000× (1× = 1 min-juego por 1 seg-real; arranca pausado). **(2) Cuadrillas** lideradas por oficial (B1/B2/B1+B2) + 1-2 helpers, una furgoneta por cuadrilla en el mapa; composición manual en Oficina; dispatch desde mapa Y modal Operaciones. **(3) Save/Load + menú de pausa estándar** (guardar / volver al menú; persiste en localStorage, **SAVE v18**). **(4) Libreas por aerolínea** (alas+cola con su color; todo el tráfico se ve igual con/sin contrato). **(5) Pernoctas realistas** (toda llegada de aerolínea basada sin salida posterior se queda; sale 06:30; 0 aviones varados). **(6) Fix botones barra** (`closest("#id")` en vez de `e.target.id`). **(7) Pop-outs por ciclo de vida de la WO**: un hito por popup EN ORDEN — callout (scope OCULTO: sistema ATA, no la avería) → cuadrilla llega → diagnóstico TSM (revela avería + tiempos + ETD) → release; pausa reloj + zoom al stand; skip por tipo (9 tipos, persistido). **Fix bug matrículas mezcladas** (release atado a la WO con matrícula sellada, no al array de aviones). Suite **1167 OK / 2 FAIL pre-existentes** (sim_reputation_segmented save-version stale + sim_scheduler_coverage overnight-V7; verificados que ya fallaban antes — NUNCA tocar). Gate runtime = Chrome headless (`shoot.mjs`) ejecutando el bundle. Release: [`builds/v0.6-line-mro.html`](builds/v0.6-line-mro.html). Commits: velocidades `8e7bfbf`, menú `0a12337`, libreas `8727d4d`, botones `70e22cd`, pernoctas `42d415d`, pop-outs `3364805`+`9b44955`.
