...
**Fase 5D acordada 2026-05-15 — Mapa esquemático animado con Pixi v8**. Decisión tomada porque tras cerrar 5A/5B/5C/extras y entregar `builds/v0.5g-final.html` (271 KB, 803/803 tests verdes), Dani identificó deuda visual del mapa que viene de F5C repurposada a eventos. Stack: **Pixi v8 sobre canvas montado dentro del panel Hangar**, GSAP/WAAPI en UI sigue intacto, sync layer game state → render state deterministic+testeable. Spec resumido: aviones como puntos luminosos con halo+trail, hangares con estado visual, partículas schematic (no fuego), bloom sutil, paleta día/noche conectada a F5A, layout crece con etapa MRO 1→4. **6 bloques P-α a P-ζ**, time-box 2 semanas. Brief detallado pendiente en `docs/BRIEF_fase5d.md`. Tauri sigue bloqueado por toolchain (irrelevante para 5D, requerido antes de Fase 6 release final). Próximo hilo Cowork: arrancar Fase 5D bloque P-α (Pixi scaffolding + sync layer).
